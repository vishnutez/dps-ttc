...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='./results_fs')
    ...
    parser.add_argument('--guid_image_idxs', type=str, default='3')
    ...
    # args.seed = args.kernel_idx
    # Set random seeds for reproducibility
    torch.manual_seed(args.seed)  # For PyTorch CPU
    # np.random.seed(args.seed)  # For NumPy
    if torch.cuda.is_available():
        torch.cuda.manual_seed(args.seed)  # For PyTorch CUDA
        torch.cuda.manual_seed_all(args.seed)  # For all CUDA devices
    ...
    guid_images_idxs = args.guid_image_idxs.split(',')
    args.n_guid_images = len(guid_images_idxs)

    guid_images_idxs_snake = args.guid_image_idxs.replace(',', '_')
    ...
    dir_name = f"norm_{args.norm_exp}" \
                ...
                + f"_guid_images_idxs_{guid_images_idxs_snake}"
    ...
    for guid_idx in guid_images_idxs:
        guid_filename = guid_idx.zfill(4)
    ...
    print('n_guid_images = ', n_recorded_images)
    ...
    for path_group_idx in range(args.n_paths // args.batch_size):
        ...
        for sample_idx in range(len(sample)):
            ...
            df = pd.DataFrame({'ref_image': [args.ref_image_idx], \
                            ...
                            'guid_images': [f"{args.guid_image_idxs}"], \
                            'project': [args.project]})
